...
KingSoa lives on the sender and receiver side, but dont worry it only has two classes.
...
* :url => url to which the request goes out in form of '[method] URL:Port'
** 'http://my_uri' =>defaults to a POST request without http verb
** 'GET http://my_uri:6969' => GET to port 6969
** 'DELETE http://my_uri'
** 'PUT http://my_uri:6669'
* :auth => optional authentication string OR integer, should also be known
(defined) on the receiving side.
...
behaviour can be customized when setting it up:
  ...
  # grabs alle requests to 
  http://my_url/incoming_soa
